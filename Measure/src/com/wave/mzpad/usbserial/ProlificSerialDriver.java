        ...
        public int read(byte[] dest, int timeoutMillis) throws IOException {
            synchronized (mReadBufferLock) {
             /*   int readAmt = Math.min(dest.length, mReadBuffer.length);
                ...
                return numBytesRead;*/
            	//在500ms下读取所有字符
                Log.d(TAG, "ProlificSerialPort mReadBufferLock: read:");
                long startTime = System.currentTimeMillis();
                long endTime = System.currentTimeMillis();   
                int len = dest.length ;
                int size = this.mReadEndpoint.getMaxPacketSize();
                int totalBytes = 0;
                int offset = 0;
                   ...
                   while ((totalBytes < len) && (endTime - startTime < timeoutMillis)) {
                       if (len - totalBytes >= size)
                           size = this.mReadEndpoint.getMaxPacketSize();
                       else {
                           size = len - totalBytes;
                       }
                       int n = this.mConnection.bulkTransfer(this.mReadEndpoint, readBuffer, size,
                               100);
                       if (n >= 0) {
                           System.arraycopy(readBuffer, 0, dest, offset, n);
                           offset += n;
                           totalBytes += n;
                       }
